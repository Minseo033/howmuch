...
- GPS 측정 정확도 품질 기준은 기존 50m 이하를 유지했다. 인증 반경을 넓히면서 위치 측정 품질까지 완화하지 않도록 거리 기준과 정확도 기준을 별도 상수로 분리했다.
- Flutter 경계 테스트를 100m 성공·100.1m 실패로 변경하고, 백엔드에 약 89m 위치의 방문 저장 성공 회귀 테스트를 추가했다. 기존 약 111m 초과 차단 테스트는 유지했다.
