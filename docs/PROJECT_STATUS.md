...
> **2026-09-13 교차검증 데이터 확장**: 행안부 전국 12,843건과 서비스 11,207건을 전화번호·정규화 상호·주소·행정구역·건물번호로 교차검증해 3,403곳을 추가 연결했다. 상세정보 커버리지는 5,762곳에서 9,165곳(고유 매장 기준 81.82%)으로 늘었다. 기존 확정 매칭 5,545곳의 주차·포장·지역화폐 값이 전부 동일하게 재현됐고, 충돌·복수 후보·근거 부족 2,037곳은 보류했다. 상세는 5-91 참조.
...
## 5-91. 9/13 행안부 전국 데이터 교차검증 확장 (로컬 검증 단계)
...
- 현재 변경은 로컬 검증 단계이며 main push, Render 배포, 운영 API 검증은 아직 수행하지 않았다.
