...
- 카카오 앱 1447987 콘솔에서 `profile_image`는 필수 동의, `account_email`은 권한 없음으로 확인했다. 이메일은 코드나 재로그인으로 해결되지 않으며 비즈 앱 등록 및 필요한 이메일 동의항목 설정이 남는다. 본인인증·약관 동의는 소유자 확인이 필요하다.
