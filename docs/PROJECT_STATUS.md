...
- 운영 Chrome에서 알림 배너를 닫을 때 Kakao 지도 위에 회색 CanvasKit 레이어가 남는 현상을 추가 재현했다. 배너가 사라져도 최상위 `Stack`을 유지하고 내부만 자리표시자로 교체해 지도 `HtmlElementView`의 합성 레이어가 재생성되지 않도록 보완했다.
