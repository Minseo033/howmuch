...
- **검증·배포**: 백엔드 전체 테스트, 카탈로그 식별자·출처 정합성 검사, `git diff --check`를 통과했다. 첫 59건 푸시의 자동 배포는 공개 별칭 갱신 직후 구 CDN 응답을 읽어 파일 해시 검증에 실패했으므로, 해시 비교를 약화하지 않고 10초 간격 최대 12회 재시도하도록 CI를 수정했다. 최종 운영 반영 결과는 배포 확인 후 기록한다.
