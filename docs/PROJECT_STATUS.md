...
## 5-67. 9/10 마이페이지·설정 실제 동작 및 UI/UX 보완

- [상세 점검 기록](SETTINGS_AUDIT_2026-09-10.md)에 기능별 실제 연결, 미지원 항목, 상태 검증과 남은 수동 검증을 기록했다.
- 서버 발송과 무관한 로컬 푸시·마케팅 스위치를 제거하고 실제 알림 설정으로 통합했다. 위치 권한은 항상 허용으로 표시하던 값을 실제 OS/브라우저 상태로 교체하고 요청·설정·재확인 화면을 추가했다. 웹 푸시 미지원, 마케팅·자동 정기 발송 미제공도 명시한다.
- 고정 좌표의 설정 UI를 흐름형 목록과 공용 설정 컴포넌트로 교체했다. 저장 전 초안, 요청 중 잠금, 실패 후 재시도, 성공 표시, 나가기 확인을 적용하고 긴 찜 목록·큰 글자의 화면 넘침을 수정했다. 공용 하단 메뉴 높이도 글자 크기를 반영한다.
- 가격 알림은 소유권을 검증한 단일 Firestore 배치로 매장 수신 여부와 계정 조건을 함께 저장한다. 찜이 없어도 실제 조건을 조회·저장하며, 다른 설정 화면에서 조건을 덮어쓰지 않는다. 기존 매장별 API는 유지한다.
- 실제 공개 범위에 반영되지 않던 프로필 스위치는 현재 공개 범위 안내로 바꾸고 닉네임 초안 이탈 확인을 추가했다. 탈퇴 동의는 기본 해제하고 최종 확인과 중복 실행 방지를 적용했다. 실제 탈퇴·운영 설정 변경·알림 발송은 검증 중 실행하지 않았다.
- 정적 분석 이슈 0건, Flutter 전체 188개, 서버 전체 179개, 설정 렌더 포함 28개 테스트, 웹 release 빌드·백엔드 JAR 생성·공백 검사를 확인했다. 실제 기기 푸시 수신과 운영 API 저장 E2E는 별도 수동 검증 대상이다.
- 코드·문서 커밋 `a3e946e`를 `main`에 푸시했다. 후속 요청으로 Vercel production 배포 `dpl_8GnD13SF9H6PLNpqVWBwMSXvRNo6`를 기존 운영 주소 `https://howmuch-zeta.vercel.app`에 연결했다. 주요 웹 파일 6개가 로컬 release 빌드와 SHA-256 기준으로 일치하고 진입 경로 6개가 HTTP 200, 보안 헤더, 백엔드 `/healthz` HTTP 200을 확인했다. 실제 계정 설정 저장과 기기 푸시 수신은 수행하지 않았다.
