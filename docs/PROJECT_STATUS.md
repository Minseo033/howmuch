...
> 최신 배포 커밋: 81c8da6 (8/11 **태관 FE 선별 이식 + 회귀 테스트**). `main == origin/main`, Render 백엔드 + Vercel 웹 배포 완료. 실서비스 전체 클릭 QA 결과는 5-16 참조
> 8/13 **다나·태관 통합 보완 작업 중**: `codex/fix-team-integration` 브랜치에 코드 반영·검증 완료, 아직 미커밋·미배포. 배포 전 필수 환경값과 검증 결과는 5-17 참조
> 8/13 **빈 리뷰 생성 차단 추가**: 같은 작업 브랜치에서 프론트 기본문구 제거·필수 입력/중복 제출 방지와 백엔드 인증·필수값 이중 검증 완료. 운영 데이터 감사 결과와 테스트는 5-18 참조
> 최신 기능 기준: 24b8be2 (자동 로그인·게스트 찜 방어·동네제보 댓글/답글/좋아요/알림 연동), 문서/회귀 테스트 후속 커밋 afa502d·81c8da6까지 push 완료 — 상세는 5-15 참조
...
## 5-17. 8/13 다나 알림 연동 + 태관 자동 로그인·제보 이미지 보완 (배포 전)
...
- 브라우저 실계정 새로고침 최종 판정은 배포 후 다시 수행해야 한다.
...
**현재 판정**: 다나와 태관의 5주차 과제는 **코드·자동 테스트 기준 완료**다. 이미지 업로드는 요금제 결정에 따라 운영 범위에서 분리했고, 자동 로그인·알림·리뷰 수정의 **실서비스 최종 판정은 배포 후 QA 전까지 보류**한다.

## 5-18. 8/13 빈 리뷰 생성 및 게스트 리뷰 경로 차단 (배포 전)
...
**배포 후 재검증**:
1. 로그인한 실계정에서 빈 폼 제출이 각 필드 오류만 표시하고 리뷰 수를 늘리지 않는지 확인한다.
2. 정상 리뷰 1건을 작성해 메뉴·결제 가격·내용·별점과 작성 uid가 저장되는지 확인한 뒤 QA 리뷰를 관리자에서 삭제한다.
3. 로그아웃 상태와 무토큰 직접 POST가 모두 401이며 관리자 리뷰 목록에 신규 `게스트`/기본문구 리뷰가 생기지 않는지 확인한다.

**현재 판정**: 5-16의 빈 리뷰 생성 문제는 **코드·자동 테스트 기준 해결**, 아직 미배포이므로 운영 최종 판정은 배포 후 실계정 QA까지 보류한다.
