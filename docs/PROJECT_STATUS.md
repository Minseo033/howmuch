...
6. ~~예상 절약 금액(2,000원) 목업~~ → ✅ 방문 인증 플로우 실데이터화 완료 (8/3): POST /api/visits 생성 + 절약 금액 서버 룰(v1 간이: 업종 평균가 − 결제가, 하한 0. 업종은 매장명으로 공공데이터 캐시에서 조회, 없으면 기본 10,000원). 인증 화면에 메뉴·금액 입력 + 예상 절약 미리보기, 완료 화면에 실제 savedAmount + 이번 달 누적(stats 재사용). TODO: 비교 기준가를 공공데이터/제보 실제 가격으로 고도화
