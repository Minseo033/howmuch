...
- Render 자동 배포는 실행됐으나 `GeminiService`의 다중 생성자 중 주입 대상을 고르지 못해 서버 기동이 실패했다. 4개 설정값 생성자에 `@Autowired`를 명시하고 독립 Spring 컨텍스트 회귀 테스트를 추가했으며, Render와 동일한 `clean test bootJar`를 통과했다.
- 운영에서 이름·본문·반려 사유에 QA/테스트가 명시된 제보 9건, 리뷰 1건, 댓글 2건을 복구용 식별 목록 작성 후 삭제했고 재조회 결과 같은 표시가 0건이었다. 일반 사용자 데이터는 삭제하지 않았다. 명시적 테스트 문의 3건은 새 API 배포 후 정리한다.
