...
> 8/13 **방문 위치 인증 개선 구현·자동 검증 완료(배포 전)**: 매장 좌표로 GPS 거리를 계산해 50m 이내만 인증하고, 서버는 인증 방식·거리만 저장한다. 기존 정적 거리와 실패 시 가짜 방문 내역을 제거했으며, 실기기 위치 권한 QA와 운영 배포는 아직 수행하지 않았다. 상세는 5-23 참조.
> 8/13 **목업 제거 1차 구현·검증 완료(배포 전)**: 검색 오류의 가짜 매장 폴백을 재시도 화면으로 교체하고, 매장 상세의 고정 별점·리뷰 수·절약액 및 추후 개발 표기와 가짜 소셜 계정 연결 데이터를 제거했다. 리뷰·로그인 정보는 기존 실데이터로 표시한다. 상세는 5-24 참조.
...
## 5-24. 8/13 목업 제거 1차 (기능 브랜치 검증 완료, 배포 전)
