...
> 최신 운영 배포 커밋: 8eb9952 (8/13 **6주차 운영 안정화**). Render 백엔드 + Vercel 웹 배포·운영 QA 완료. GitHub `main`은 `19b9c66`까지 반영됐으며, 아래 지도 반응성·문의 답변 기능은 민서 브랜치에서 QA 완료 후 main 반영 대기다. 상세는 5-17~5-26 참조
...
> 8/13 **내 위치 반응성·문의 답변 흐름 구현·QA 완료(main 반영 대기)**: 캐시 위치 우선 이동으로 지도 버튼의 첫 반응을 개선하고, 어드민 답변 → 사용자 문의 내역·알림함 확인까지 연결했다. 웹은 앱 안 알림함을 지원하되, 브라우저 종료 후 시스템 푸시는 아직 지원하지 않는다. 상세는 5-26 참조.
...
## 5-26. 8/13 지도 내 위치 반응성 및 문의 답변 흐름 (민서 브랜치 QA 완료, main 반영 대기)
