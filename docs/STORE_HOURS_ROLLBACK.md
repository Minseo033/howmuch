...
작업 검증은 별도 작업 공간에서 진행했다. 검증 후 기능 커밋 `4a1047b5893fe274a06dff6b9d7729e055ce95ff`를 main에 fast-forward 통합하고 푸시·배포했다. 운영 DB는 직접 변경하지 않았다. 이번 영업시간 자료는 공공데이터 원본과 별도 파일로 관리한다.
...
배포 완료 후에도 위 이전 배포를 보존했다. 새 웹 배포는 `dpl_7BGQMVv2RgPpYHKNvgQzb1GtAdRV`; 운영 API 39곳 및 대표 웹 13개 검증 항목 통과. 되돌릴 기능 커밋은 `4a1047b`이며 후속 문서 기록 커밋과 구분한다.
