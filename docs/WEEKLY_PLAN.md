...
> **8/13 민서(PM) 6주차 선행 개발 — `codex/week6-pm-operations` 구현·로컬 검증 완료, 운영 반영 전**
...
> - **운영 반영 전 필수**: GitHub 저장소 Secret `ADMIN_KEY` 등록, Render `PUBLIC_DATA_API_KEY` 등록 및 기존 노출 키 교체, Firebase 프로젝트 확인 후 `firebase deploy --only firestore:rules`, 브랜치 리뷰·main 병합·배포가 남아 있다.
