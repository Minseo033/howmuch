...
> - **8/13 배포 결정**: Firebase Storage는 Blaze(종량제) 전환이 필요해 이번 릴리스에서는 Spark를 유지한다. 제보 이미지 코드는 보존하되 `REPORT_IMAGE_UPLOAD_ENABLED` 기본값을 `false`로 두고 사진 선택 UI와 업로드 호출을 함께 비활성화한다.
> - **릴리스 게이트 완료**: `623146d`를 Render/Vercel에 배포하고 `test123` 실계정으로 자동 로그인 새로고침·알림 목록/개별 읽음/설정 저장·빈 리뷰 차단·유효 리뷰 저장/원복을 확인했다. 실제 이미지 업로드는 6주차 Storage 방식 결정 후 별도 게이트로 이관했다.
