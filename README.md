...
- **[NEW]** 카카오 로그인 및 딥링크 라우팅 완벽 연동: iOS 환경에서 카카오톡 앱 로그인 복귀 시 발생하는 `GoRouter`의 스플래시 화면 초기화(리셋) 버그를 완벽하게 해결했습니다. 백그라운드 로그인 로딩 화면 처리와 권한 설정 화면으로의 매끄러운 트랜지션을 구현했습니다.
- **[NEW]** 검색 및 필터 화면 UX 고도화: 검색 화면 진입 후 텍스트 검색 시 텍스트만 지우고, 필터 적용 시 맵 화면으로 돌아갔을 때 필터 상태가 정상 유지되도록 State 분리 로직을 최적화했습니다.
- **[NEW]** 홈 지도 UX/UI 개선: 기존 우측 하단의 작은 플로팅 버튼을 제거하고, 피그마(Figma) 디자인 시스템에 맞춰 하단에 꽉 차는 예쁜 Glassmorphism(블러) 스타일의 `지도에서 보기` 및 `검색하기` 버튼 네비게이션을 신규 구현했습니다.
- **[NEW]** 프론트엔드 - 백엔드 통신 및 예외처리 점검: `ngrok` 환경에서 Spring Boot 서버와의 CORS 및 브라우저 경고 헤더 우회 테스트를 성공적으로 마무리했습니다.
...
최근 지도 마커 렌더링 성능 개선을 위해 백엔드와 프론트엔드 통신 방식이 크게 변경되었습니다. **코드 풀(Pull) 후 아래 2가지를 반드시 확인하세요!**

1. **백엔드(Spring Boot) 필수 재빌드/재실행**
   - 백엔드에 11,000개 매장 데이터를 서버 시작 시 메모리에 캐싱하는 기능(`FirebaseService.java` 내 `@PostConstruct`)과 `/api/test/all` 엔드포인트가 추가되었습니다.
   - 프론트엔드 코드만 최신으로 받고 구버전 백엔드를 켜두면 지도에 마커가 하나도 뜨지 않습니다. **반드시 백엔드 코드를 풀 받고 껐다 켜서 최신화**해주세요.

2. **프론트엔드 통신 터널 주소 수정 (`home_map_screen.dart` 127번째 줄)**
   - 현재 `home_map_screen.dart`의 127줄에는 **특정 개발자의 ngrok 로컬 터널 주소(`https://sulfurously-transhumant-dennise.ngrok-free.dev`)가 하드코딩**되어 있습니다.
   - 코드를 그대로 실행하면 해당 개발자 PC의 서버로 요청이 가게 됩니다.
   - **iOS 시뮬레이터 사용자:** `http://localhost:8081` 로 주소 변경
   - **Android 에뮬레이터 사용자:** `http://10.0.2.2:8081` 로 주소 변경
   - **본인 실기기(폰) 테스트:** 각자 본인 PC에서 `ngrok http 8081` 실행 후 발급받은 본인만의 터널 주소로 변경하여 사용하세요.


## 🚀 현재 프론트엔드 진행 상황 요약 (최신 업데이트)

### 1. 홈 화면 (지도)
- 카카오맵 웹뷰 연동 완료 및 11,000개 공공데이터(착한가격업소 등) 마커 렌더링 최적화.
- 화면 이동/축소 시 보이는 지도 영역(Bounds)에 맞춰 동적으로 마커 로딩.
- 바텀 네비게이션 드래그 시 지도 딸림 현상 수정 (UX 개선).
- 지도 빈 공간 탭 시 활성화된 가게 정보 카드 닫기 기능 추가.
- 마커 터치 시 튀는 색상(빨간색 등) 강조 효과 제거.

### 2. 검색 화면
- **실시간 검색(Search-as-you-type)** 구현: 타이핑 시 0.3초 딜레이(디바운스) 후 즉시 결과 반영.
- **거리순 기본 정렬**: 현재 사용자 위치 기반 최단 거리순으로 100개 노출 제한(UI 버벅임 방지).
- **거리 표시 UI**: 검색 결과 카드에 `500m`, `1.5km` 형태로 내 위치와의 거리 표시.
- **지역명(주소) 검색 포함**: 가게명, 메뉴뿐만 아니라 주소(`address`) 필드까지 포함하여 지역 검색 지원.
- 7-2 엣지케이스(검색 결과 없음) 화면 Figma 시안에 맞춰 적용.

### 3. 로그인 및 인증 플로우
- 이메일/소셜 로그인 화면 퍼블리싱 (기능은 추후 백엔드/Firebase 연동 예정).

### ⏳ 향후 개발 예정 기능 (UX/UI 우선)
- 앱 첫인상을 담당하는 온보딩(Onboarding), 스플래시(Splash) 화면 UI 퍼블리싱.
- 비밀번호 찾기 및 회원가입 완료 축하 모달 구현.
- 마이페이지(찜하기, 프로필 관리) 화면 퍼블리싱.
- (백엔드 완료 시) 가게 상세 페이지 내부 리뷰/댓글 쓰기 기능 구현 및 커뮤니티(제보) 기능 연동.
